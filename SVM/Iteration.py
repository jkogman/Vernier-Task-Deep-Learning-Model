...
def iteration(d, rate, D, T, sig_ksi, t_size, x_size, x_max, sig, prior, n_sigs):
    ...
    S1 = np.zeros((int(x_size), int(t_size)), dtype=np.int_)
    S2 = np.zeros((int(x_size), int(t_size)),  dtype=np.int_)
    ...
    # Reshaping the spike matrices into a vector

    S1 = np.reshape(S1 , np.size(S1))
    S2 = np.reshape(S2 , np.size(S2))
    data_vec = np.concatenate((S1 , S2))
